...
function SuggestedDoctorCard({doctorAgent, setSelectedDoctor,selectedDoctor}:props) {
    ...
    return (
        ...
        // hover:border-blue-500 cursor-pointer${selectedDoctor?.id==doctorAgent?.id&&'border-blue-500'}`} onClick={()=>setSelectedDoctor(doctorAgent)}
        // >
        <div
  className={`flex flex-col items-center border rounded-2xl shadow p-5 cursor-pointer 
    hover:border-blue-500 
    ${selectedDoctor?.id === doctorAgent?.id ? 'border-blue-500' : ''}`}
  onClick={() => setSelectedDoctor(doctorAgent)}  // calls parent function
>
